...
VertexIndex gpu_computeGLVertexID(
	...
	if (indices == NULL) { //Indexing není aktivní,
		return vertexShaderInvocation;
	}
	else { //Indexing je aktivní
...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  return (void const*)(head->offset + head->stride * (size_t)gl_VertexID + (size_t)head->buffer);
}
...
void gpu_runVertexPuller(
	...
	for (size_t a = 0; a < MAX_ATTRIBUTES; a++) {
		const GPUVertexPullerHead *head = &puller->heads[a];
		if (head->enabled != 0) {
			output->attributes[a] = gpu_computeVertexAttributeDataPointer(head, id);
		}
		else {
			output->attributes[a] = NULL;
		}
	}
}
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  float invDET = 1.0f / ((vertices[1].data[1] - vertices[2].data[1]) * (vertices[0].data[0] - vertices[2].data[0]) +
	  (vertices[2].data[0] - vertices[1].data[0]) * (vertices[0].data[1] - vertices[2].data[1]));


  coords->data[0] = ((vertices[1].data[1] - vertices[2].data[1]) * (pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0]) * (pixelCenter[0].data[1] - vertices[2].data[1])) * invDET;
  coords->data[1] = ((vertices[2].data[1] - vertices[0].data[1]) * (pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[0].data[0] - vertices[2].data[0]) * (pixelCenter[0].data[1] - vertices[2].data[1])) * invDET;
